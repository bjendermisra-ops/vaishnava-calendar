/**
 * =====================================================================
 * ISKCON GCAL PURE VEDIC CALCULATION ENGINE
 * 100% Sunrise-to-Sunrise (Ahoratra) & Hari-bhakti-vilasa Conformance
 * =====================================================================
 */
...
(function (root, factory) {
  ...
  const NAKSHATRAS = [
    ...
    "Uttara-phalguni (Abbyakta)", "Hasta", "Chitra", "Swati", "Visakha", "Anuradha",
  ...
  const EKADASHI_NAMES = {
    ...
    "5_0": "PARSHVA", "5_1": "ANNADA",
    "6_0": "PASANKUSA", "6_1": "INDIRA",
    "7_0": "Utthana", "7_1": "RAMA",           // Damodara Krishna: RAMA
  ...
  /**
   * Vedic Time Formatter with Nearest-Second Rounding (GCal Standard)
   */
  function formatUtcToVedic(astroDate, tzOffsetHours) {
    ...
    // GCal Minute Rounding: >= 30 seconds rounds up
    const roundedMinutes = Math.round(localSec / 60) % 1440;
    const roundH = Math.floor(roundedMinutes / 60);
    const roundM = roundedMinutes % 60;
    
    return {
      strHM: `${String(roundH).padStart(2, '0')}:${String(roundM).padStart(2, '0')}`,
      strHMS: `${String(hh).padStart(2, '0')}:${String(mm).padStart(2, '0')}:${String(ss).padStart(2, '0')}`,
      totalMinutes: roundedMinutes,
      exactSeconds: localSec,
      dateObj: jsDate
    };
  }
  ...
  function findTithiEndTime(startDateUtc, targetBoundaryDeg, tzOffset) {
    ...
    const endFmt = formatUtcToVedic(endUtcDate, tzOffset);
  ...
  function calculateDay(date, loc) {
    ...
    // 1. Earth-sky (Visible) Sunrise & Sunset
    ...
    const sunRiseEarthFmt = formatUtcToVedic(riseVisible, loc.tz);
    const sunSetEarthFmt = formatUtcToVedic(setVisible, loc.tz);

    // 2. Solar Noon
    const noonSec = (riseVisible.date.getTime() + setVisible.date.getTime()) / 2;
    const noonFmt = formatUtcToVedic(new Date(noonSec), loc.tz);

    // 3. GCAL Celestial Horizon (Noon +/- 360 min)
    ...
    // 5. VEDIC SUNRISE-TO-SUNRISE MOON EVENTS:
    // A Vedic day begins at today's sunrise and ends at tomorrow's sunrise
    const sunriseUtc = riseVisible ? riseVisible.date : localNoonUtc;
    const moonRise = Astronomy.SearchRiseSet("Moon", obs, +1, sunriseUtc, 1.0);
    const moonSet = Astronomy.SearchRiseSet("Moon", obs, -1, sunriseUtc, 1.0);

    const moonRiseStr = moonRise ? formatUtcToVedic(moonRise, loc.tz).strHM : "--:--";
    const moonSetStr = moonSet ? formatUtcToVedic(moonSet, loc.tz).strHM : "--:--";

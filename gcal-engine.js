/**
 * =====================================================================
 * ISKCON GCAL CORE CALCULATION ENGINE (Universal Mathematical Engine)
 * Pure First-Principles Astronomy & Hari-bhakti-vilasa Logic
...
(function (root, factory) {
  ...
  const EKADASHI_NAMES = {
    ...
    "5_0": "PARSHVA", "5_1": "ANNADA",       // Hrishikesha: Gaura=PARSHVA, Krishna=ANNADA
    "6_0": "PASANKUSA", "6_1": "INDIRA",     // Padmanabha: Gaura=PASANKUSA, Krishna=INDIRA
  ...
  /**
   * Chitra-Paksha (Lahiri) Ayanamsha (Standard IAU Precession)
   */
  ...
  /**
   * Pure UTC to Local Time Formatter (Timezone-Independent)
   */
  ...
  /**
   * Calculate Tithi Phase (Moon Longitude - Sun Longitude)
   */
  ...
  function determineVaishnavaMasa(targetDateUtc) {
    ...
    // Approximate time to Amavasya (diffDegrees = 0)
    ...
    // Binary search to find exact Amavasya moment
  ...
  function calculateDay(date, loc) {
    ...
    const riseVisible = Astronomy.SearchRiseSet("Sun", obs, +1, utcNoon, -1);
    const setVisible = Astronomy.SearchRiseSet("Sun", obs, -1, utcNoon, 1);
    ...
    // 3. Exact Celestial Horizon using Spherical Trigonometry:
    // cos(H) = -tan(latitude) * tan(declination)
    const sunEquator = Astronomy.Equator("Sun", utcNoon, obs, true, true);
    const latRad = loc.lat * (Math.PI / 180);
    const decRad = sunEquator.dec * (Math.PI / 180);
    const cosH_cel = -Math.tan(latRad) * Math.tan(decRad);
    const H_cel_deg = Math.acos(Math.max(-1, Math.min(1, cosH_cel))) * (180 / Math.PI);
    const H_cel_minutes = (H_cel_deg / 15) * 60;

    const noonMinutes = noonFmt.totalMinutes;
    const celRiseMinutes = Math.round((noonMinutes - H_cel_minutes + 1440) % 1440);
    const celSetMinutes = Math.round((noonMinutes + H_cel_minutes) % 1440);
    ...
    // 5. Exact Moonrise & Moonset for this 24-hour civil day
    const utcMidnight = new Date(Date.UTC(y, m, d, Math.floor(0 - loc.tz), 0, 0));
    const moonRise = Astronomy.SearchRiseSet("Moon", obs, +1, utcMidnight, 1.2);
    const moonSet = Astronomy.SearchRiseSet("Moon", obs, -1, utcMidnight, 1.2);
    ...
    if (tithiIndex === 10 || tithiIndex === 25) {
      ...
      // Check for Sri Vamana Dvadashi (Appearance of Lord Vamanadeva)
